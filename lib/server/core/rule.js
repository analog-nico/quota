...
Rule.prototype.isAvailable = function (managerName, scope, resources, options, fastLane) {
...
Rule.prototype.enqueue = function (managerName, scope, resources, options) {
    ...
    return new BPromise(function (resolve, reject) {
        ...
        if (options && _.isFinite(options.maxWait)) {
            scopeBundle.validUntil = (new Date()).getTime() + options.maxWait;
        }

        self._getBundleForScope(managerName, scope).queueing.add(scopeBundle);
        ...
        if (!_.isUndefined(this._Queueing)) {
            ...
            scopeBundle.moreAvailable = function () {
                ...
                while (true) {
                    ...
                    var entry = scopeBundle.queueing.next();
                    if (_.isUndefined(entry)) {
                        return;
                    }

                    if (entry.validUntil && entry.validUntil < now) {
                        entry.reject(new errors.OutOfQuotaError(managerName));
                        return;
                    }
