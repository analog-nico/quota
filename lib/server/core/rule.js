...
Rule.prototype.isAvailable = function (scope, resources, options, fastLane) {

    var scopeBundle = this._getBundleForScope(scope);

    if (!_.isUndefined(this._Queueing) && fastLane !== true) {
...
Rule.prototype.enqueue = function (scope, resources, options) {
    ...
    return new BPromise(function (resolve) {
        ...
        self._getBundleForScope(scope).queueing.add(resolve);
...
Rule.prototype.reserve = function (scope, resources, options) {
    var resourceAmount = this._getResourceAmount(resources);
    return this._getBundleForScope(scope).throttling.reserve(resourceAmount, options);
};
...
Rule.prototype._getBundleForScope = function (scope) {
    ...
    if (!this._throttling[scopeString]) {
        ...
        if (!_.isUndefined(this._Queueing)) {
            ...
            scopeBundle.moreAvailable = function () {
                var resolve = scopeBundle.queueing.next();
                if (resolve) {
                    resolve();
                }
            };
